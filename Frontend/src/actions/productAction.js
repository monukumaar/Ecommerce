  ...
  (keyword = "", currentPage = 1, price = [0, 5000000], category = "", ratings = 0) =>
    async (dispatch) => {
      try {
        ...
        // console.log("FETCHING PRODUCTS >>> ", { keyword, currentPage, price });
...
export const getAdminProduct = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get("http://localhost:3000/api/v1/admin/products", { withCredentials: true });
...
export const deleteProduct = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(`http://localhost:3000/api/v1/admin/product/${id}`, { withCredentials: true });
...
export const deleteReviews = (reviewId, productId) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(
      `http://localhost:3000/api/v1/reviews?id=${reviewId}&productId=${productId}`
    );
